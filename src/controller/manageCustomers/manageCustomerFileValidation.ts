...
import { customerDetail } from './manageCustomers';

function manageCustomerValidation(customerDetails: customerDetail) {
  let customerFormErrorObject: { [key: string]: string } = {};
    ...
    switch (key) {
      case 'firstname':
        if (!customerDetails[key]) {
          ...
          customerDetails[key].trim().length === 0 &&
          customerDetails[key] !== ''
        ) {
      ...
      case 'email':
        ...
        if (!customerDetails[key]) {
          ...
          !regexemail.test(customerDetails[key]) &&
          customerDetails[key] !== ''
        ) {
      ...
      case 'phonenumber':
        if (!customerDetails[key]) {
          ...
          isNaN(parseInt(customerDetails[key].toString())) ||
          customerDetails[key].length !== 10 ||
          customerDetails[key].trim().length === 0
        ) {
      ...
      case 'address':
        if (customerDetails[key] && customerDetails[key].trim().length === 0) {
          customerFormErrorObject[key] =
            '* Please Enter Address Not Only Space';
        } else {
          delete customerFormErrorObject[key];
        }
        break;
...
export default manageCustomerValidation;

...
import {
  ...
  storageDetails,
} from '../../service/report/reportPdf';
import { RowDataPacket } from 'mysql2';

async function reportPdfPage(req: Request, res: Response) {
...
async function generatePdf(data: {}, reportType: string) {
  const templatePath = path.join(
    __dirname,
    '../../views/reports/pdfTemplate/productPdfTemplate.ejs'
  );
  const template = fs.readFileSync(templatePath, 'utf8');
  const html = ejs.render(template, { data: data });
  let browser = await puppeteer.launch();
  ...
  const pdfPath = path.join(
    __dirname,
    `../../public/uploads/pdfFiles/${fileName}`
  ); //path of pdf
...
async function productReportGenerate(req: Request, res: Response) {
  try {
    ...
    const productDetailsArray = await productGenerateReport(
      req.body,
      storageId
    );
    ...
    if (productDetailsArray.length > 0 && storageDetailsArray.length > 0) {
      const productDetailsObject: { [key: string]: string | RowDataPacket[] } =
        {};
    ...
    logError('Product Generate Report: ' + error);
    return res.status(500).json({ message: 'Something Went Wrong..' });
  }
}

async function outOfStockProducts(req: Request, res: Response) {
  try {
    let storageId;
    const roleId = req.user?.roleId;
    if (roleId === 4) {
    ...
    const productOutOfStockArray = await productOutOfStockGenerateReport(
      req.body,
      storageId
    );
    const storageDetailsArray = await storageDetails(storageId);

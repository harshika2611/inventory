...
async function getProductsByCategory(id: string | number) {
  try {
    const [results] = await connection.execute(
...
async function fetchPurchaseOrder(data: { [key: string]: number | string }) {
  try {
...
async function createPurchaseOrder(data: { [key: string]: string | number }) {
  try {
    const [results] = await connection.execute(
...
async function updatePurchaseOrder(data: { [key: string]: string | number }) {
  try {
    const [results] = await connection.execute(
...
async function fetchPurchaseOrderView(req: Request<{}, {}, {}, View>) {
  try {

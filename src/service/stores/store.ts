...
import { Istore } from '../../controller/stores/store';

async function insertStoreQuery(body: Istore) {
  try {
...
async function getStoreQuery() {
  try {
    ...
    logError('Get Stores: ' + error);
    return [];
  }
}

async function checkStoreExistQuery(storeId: string | undefined) {
  try {
    ...
    const [result] = await connection.execute<RowDataPacket[]>(checkStore, [
      storeId,
    ]);
    // console.log(result);
    // console.log(storeId);
    ...
    logError('Check store: ' + error);
    throw error;
  }
}

async function updateStoreQuery(body: Istore, storeId: number) {
  try {
    // console.log(storeId);
...
async function deleteStoreQuery(storeId: string | undefined) {
  ...
  try {
    ...
    logError('Delete Store: ' + error);
  }
}

async function deleteStoreProductQuery(
  ...
  try {
    ...
    logError('Delete Store Product: ' + error);
  }
}

async function storeProductQuery(storeId: string | undefined) {
  try {
    ...
    const [storeResult] = await connection.execute<RowDataPacket[]>(
      storeProducts,
      [storeId]
    );
    return storeResult;
  } catch (error) {
    logError('Store Product : ' + error);
  }
}
export {
  insertStoreQuery,

...
const getOrderreport = async (storage: number | null) => {
...
const getApiordersProduct = async (id: string) => {
  ...
  return await connection.execute<RowDataPacket[]>(sql, [id]);
};

const getOrderreportBydate = async (
  fromDate: string,
  toDate: string,
  storage: number | null
) => {
...
const getOrderDayreportDay = async (date: string, storage: number | null) => {
...
const getOrderDayreportMonth = async (
  month: string,
  storage: number | null
) => {

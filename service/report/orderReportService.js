...
const getOrderreportBydate = async (fromDate, toDate, storage) => {
  let sql = `	SELECT sales_order.id as Order_Id ,sales_name as Order_Name ,customer_master.firstname as Costomer_name,option_master.value as Order_Status,amount as Order_Amount,pay.value as Payment_Status ,order_date,sales_order.order_date as order_Time  FROM sales_order left join customer_master on sales_order.customer_id=customer_master.id left join option_master on sales_order.type=option_master.id left join option_master as pay on sales_order.payment_status=pay.id where sales_order.order_date BETWEEN ? and ? `;
  // console.log(sql);
  if (storage) {
    return await connection.execute(
...
const getOrderDayreportDay = async (date, storage) => {
  ...
  customer_master.firstname as Costomer_name,
